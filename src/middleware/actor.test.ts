...
import { ActorMiddleware, makeActorMiddleware } from "./actor";

describe("make_actor_middleware", () => {
  it("domain__header_field_present__sets_actor_and_calls_next", () => {
    const middleware = makeActorMiddleware("x-actor-id");
  ...
  it("boundary__optional_and_missing__passes_without_actor", () => {
    const middleware = makeActorMiddleware("x-actor-id", "header", false);
  ...
  it("domain__query_source__reads_actor_from_query", () => {
    const middleware = makeActorMiddleware("actor", "query");
  ...
  it("domain__custom_missing_payload__uses_configured_error", () => {
    const middleware = makeActorMiddleware(
      "actor",
      "body",
      true,
      403,
