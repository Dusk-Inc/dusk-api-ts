...
import { makeWellKnownRouter, WellKnownRouterConfig } from "./well_known.routes";
...
const buildConfig = (
  overrides: Partial<WellKnownRouterConfig> = {}
...
const buildApp = (config: WellKnownRouterConfig) => {
  const app = express();
  app.use(makeWellKnownRouter(config));
  return app;
};
